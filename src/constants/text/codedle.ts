...
export const codedleText = {
  ...
  description: (prize: string, length: number, cost: string, unix: number) =>
    ...
    `Only you can see your guesses and hints. Press **My guesses** to see them again.`,
  guessButton: 'Guess',
  mineButton: 'My guesses',
  modalTitle: 'Codedle',
  ...
  /** One guess on the board: its hint squares, the digits, and who guessed ('' on a player's own board). */
  boardLine: (squares: string, guess: string, user: string) => `${squares} \`${guess}\`${user === '' ? '' : ` ${user}`}`,
  /** Shown under the board when older guesses are hidden. */
  olderGuesses: (count: number) => `...and ${count} older ${count === 1 ? 'guess' : 'guesses'}`,
  /** A player's own board, shown only to them. */
  mineTitle: (count: number) => `**Your guesses** (${count})`,
  mineEmpty: "You haven't guessed yet.",
  ...
  wrong: (squares: string, guess: string) => `${squares} \`${guess}\` Not quite. Only you can see this hint.`,
